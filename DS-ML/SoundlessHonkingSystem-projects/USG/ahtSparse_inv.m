...
function Smooth_Waves = ahtSparse_inv(inpMat,thr,points,sep,fc_up,fc_botm)

% To invert
inpMat_inv = flipud(inpMat);

[rows,cols] = size(inpMat_inv);
N =rows;
...
for c=1:cols
    ...
    % code for coefficients at the window
        for j = 1:rows
            yi(k,c) = yi(k,c) + inpMat_inv(j,c)*bi(j,k);
            yq(k,c) = yq(k,c) + inpMat_inv(j,c)*bq(j,k);
        end
    end
