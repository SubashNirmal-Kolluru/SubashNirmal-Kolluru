...
g_movies <- c()
t_wt <- c()
dir_data <- data.frame(Directors=c(),Rating=c(),Votes=c(),Rev_score=c(),Budget=c(),Gross=c(),Critics_Rating=c())
...
for (i in 1:length(directors)){
  directors1 <- strsplit(directors[i],",")
  directors1 <- unlist(directors1)
  for (j in directors1){
    dir_data1 <- data.frame(Directors=j,Rating=ratings[i],Votes=votes[i],Rev_score=sen_score[i],Budget=budget[i],Gross=gross[i]
                            ,Critics_Rating=cri_rating[i])
    dir_data <- rbind(dir_data,dir_data1)
  }
}

dir_data <- ddply(dir_data, .(Directors,Rating,Votes,Rev_score,Budget), summarize,Gross = sum(Gross),
                  Critics_Rating=sum(Critics_Rating))

directors <- unlist(dir_data[1])
gross <- unlist(dir_data[6])
for (j in directors){
  n_movies <- c(n_movies,length(directors[directors==j]))
}
for (i in 1:length(directors)){
...
dir_data <- data.frame(Directors=directors,No_movies=n_movies,Rating=ratings,Votes=votes,Rev_score=rev_score,Budget=budget,Gross=gross
                       ,Critics_Rating=cri_rating,Grossp_movie=g_movies)

dir_data <- ddply(dir_data, .(Directors,No_movies), summarize,Rating=sum(Rating),Votes=sum(Votes),Rev_score=sum(Rev_score),Budget=sum(Budget),
                  Gross = sum(Gross),Critics_Rating=sum(Critics_Rating),Grossp_movie=sum(Grossp_movie))

db <- dbConnect(SQLite(),dbname='data.sqlite')
dbWriteTable(db,'Directors_Beta2',dir_data,overwrite=TRUE)
...
dir_data2 <- data.frame(Directors=directors,No_movies=n_movies1,Rating=ratings,Votes=votes,Rev_score=rev_score,Budget=budget,Gross=gross
                        ,Critics_Rating=cri_rating,Grossp_movie=g_movies)



wt <- c(.19667,.08343,.03645,.27785,.13871,.04952,.03453,.16)
dir_data3 <- data.frame(Director=c(),Weightage=c())
for (i in 1:length(directors)){
  if (n_movies[i]>3){
  t_wt <- wt[1]*ratings[i]+wt[2]*votes[i]+wt[3]*rev_score[i]+wt[4]*budget[i]+wt[5]*gross[i]+wt[6]*cri_rating[i]+(wt[7]+wt[8])*g_movies[i]
  dir_data1 <- data.frame(Director=directors[i],Weightage=t_wt)
  dir_data3 <- rbind(dir_data3,dir_data1)
  }
}

DT <- as.data.table(dir_data3)
director_rank <- DT[,DirectorRank:=rank(-Weightage,ties.method="first"),]

dbWriteTable(db,'Directors_ranking',director_rank,overwrite=TRUE)