function output = features(waves, sep, thr, BL, BL_factor )
...
output = zeros(1,9);            % Will increase the length 
...
%       fprintf('\n\n%s%d\n','Features for peak No:     ',peak_count - 1);
      ...
      RI = (pre_peak_value - Diastole )/pre_peak_value;
      PI = (pre_peak_value - Diastole )/(pre_peak_value + Diastole )*0.5;
      fprintf('\n%s%f\n','Acceleration: ',accel);   
      fprintf('%s%3.0f\n','Period: ',period);
      fprintf('%s%f\n','SBI: ',SBI);
      fprintf('%s%f\n','SDR: ',SDR); 
...
%       i = i - 1;
  ...
  if wave_bot(c) < min_bot && wave_bot(c) ~= 0
      min_bot = wave_bot(c);
      min_bot_location = c;      
  end
